...
The research goal of this paper was that they tested if it is possible to predict the severity of a reported bug by analysing its textual description by using a text mining algorithm. They came up with four subsidiary research questions which need to be included to find an explanation of their research goal.

\begin{itemize}
\item Concerning potential indicators of the severity of a bug, the first research question was: "Which terms in the textual descriptions of a bug report could serve as good indicators of the severity?" (Lamkanfi et al., 2010)

\item Concerning the length of a bug report, the second research question was: "Which (text) fields in the bug reports serve as the best prediction basis? The one-line summary which briefly focusses on the problem or the longer full description which includes more detail?" (Lamkanfi et al., 2010)

\item Concerning the scope of the training period, their third research question was: "How many samples must be collected before one can make a reliable predictor?" (Lamkanfi et al., 2010)

\item And finally, concerning the components of a software system, the fourth research question was: "Is it better to have a specialized predictor for each component of the software system, or can we combine bug reports over different software components (the so-called “cross- component” approach)?" (Lamkanfi et al., 2010)
\end{itemize}

The structure of the paper is as follows. In the second section "Background" we will explain all the necessary background knowledge on bug triaging and text mining. In the third section "Case Study" we will first explain the approach step by step, then we present the measures we used to process and evaluate the data and finally we will present the results. In the fourth Section "Subsidiary Research Questions" we will answer step by step the subsidiary research questions. In the fifth section "Threats to Validity", we will need to identify factors that could threaten our results and the measures we took to minimize the risk of bad results. In The sixth section "Related Work" we will discuss related work of other researchers and their findings to compare it with our findings. Finally, in the seventh section "Conclusion and Future Work" we will summarize our all findings and mention future work to be done in conjunction with our research.
